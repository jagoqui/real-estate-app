...
export function invertObject<TObject extends {[Key in keyof TObject]: string}>(
  ...
  for (const key in obj) {
    ...
    result[value as TObject[keyof TObject]] = key as {
      [Key in keyof TObject]: TObject[Key] extends TObject[keyof TObject] ? Key : never;
    }[keyof TObject];
  }
...
function cleanObject<T>(obj: NullablePartial<T>, removeNulls: boolean): Record<string, unknown> {
