import type { UpdatePropertyStatusCommand } from '@/modules/shared/domain/commands/property.commands';
...
export const updatePropertyStatusAdapter = async (args: UpdatePropertyStatusCommand): Promise<Property> => {
  const url = `${PROPERTY_ENDPOINTS.BY_ID(args.propertyId)}/status`;

  const propertyResponseDto = await api.patch<PropertyResponseDto>(url, { json: { status: args.status } }).json();
