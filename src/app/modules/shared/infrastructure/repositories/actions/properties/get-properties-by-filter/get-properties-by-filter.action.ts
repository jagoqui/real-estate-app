import type { GetPropertiesByFilterInput } from '@/modules/shared/domain/inputs/property.input';
...
export const getPropertiesByFilterAction = async (input: GetPropertiesByFilterInput): Promise<Array<Property>> => {
  const url = `${PROPERTY_ENDPOINTS.ROOT}/filter`;

  const propertiesResponseDto = await api
    .get<Array<PropertyResponseDto>>(url, { searchParams: input as Record<string, string> })
    .json();
